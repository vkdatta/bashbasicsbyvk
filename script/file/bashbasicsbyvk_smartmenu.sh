...
#  shims the menu-output/input builtins:
#
#    echo / printf  record whatever they emit into a small ring
#                   buffer, then emit it unchanged.
#    builtin printf is routed through the same printf shim.
...
#
#  So any function that prints "1) ..." and then calls
...
echo() {
  ...
  [[ $_sm_on == 1 && -t 1 ]] || { command builtin echo "$@"; return $?; }
  ...
  __s="$*"
  [ "$__e" -eq 1 ] && command builtin printf -v __s '%b' "$__s"
  [ "$__n" -eq 1 ] || __s+=$'\n'

  command builtin printf '%s' "$__s"
  _sm_record "$__s"
  return 0
}
...
# builtin shim
# ------------------------------------------------------------
# `builtin printf ...` normally bypasses the printf() shell-function shim.
# Intercept only that one form so menu output is captured by Smart Menu too.
# Every other builtin is passed straight to the real Bash builtin.
builtin() {
  if [[ "$1" == "printf" ]]; then
    shift
    printf "$@"
...
_sm_classify_mode() {
  if [ -n "${VK_MENU_MODE:-}" ]; then
    _sm_mode="$VK_MENU_MODE"
    return
  fi
...
_sm_split_command() {
  ...
  # followed by a digit, e.g. s1-5
  # only strip letters we know are command prefixes, so a stray
  # leading digit is never touched
...
_sm_input_fn() {
  command builtin printf '\r\033[K%s%s' "$_sm_prompt" "$_buf"

  local back=$(( ${#_buf} - _pos ))
  [ "$back" -gt 0 ] && command builtin printf '\033[%dD' "$back"
...
_sm_cursor_number() {
  ...
  while [ "$s" -gt 0 ] && [[ "${_buf:s-1:1}" =~ [0-9] ]]; do
    s=$((s-1))
  done

  while [ "$e" -lt "${#_buf}" ] && [[ "${_buf:e:1}" =~ [0-9] ]]; do
    e=$((e+1))
  done
...
# In multi/command mode the arrows do not replace the buffer —
# they just move a "cursor row" and toggling is done with space/enter.
...
_sm_run() {
  ...
  if (( up > 0 )); then
    command builtin printf '\r\033[%dA\033[J' "$up"
  else
    command builtin printf '\r\033[J'
  fi
  ...
  while true; do
    ...
    if [[ "$key" == $'\x1b' ]]; then
      ...
      if $multi; then
        ...
        case "$_kname" in
          ...
          up)
            _sm_move_cursor "$(_sm_next_selectable \
              $(( ${_hl_index:-1} <= 1 ? n : _hl_index - 1 )) -1)"
            ;;

          down)
            _sm_move_cursor "$(_sm_next_selectable \
              $(( ${_hl_index:-0} >= n ? 1 : _hl_index + 1 )) 1)"
            ;;

          pgup)
            _sm_move_cursor "$(_sm_next_selectable \
              $(( ${_hl_index:-1} - ${_vp_page_step:-10} < 1 \
                ? 1 \
                : _hl_index - _vp_page_step )) 1)"
            ;;

          pgdn)
            _sm_move_cursor "$(_sm_next_selectable \
              $(( ${_hl_index:-0} + ${_vp_page_step:-10} > n \
                ? n \
                : _hl_index + _vp_page_step )) -1)"
            ;;

          home)
            _sm_move_cursor "$(_sm_next_selectable 1 1)"
            ;;

          end)
            _sm_move_cursor "$(_sm_next_selectable "$n" -1)"
            ;;

          right)
            if [ "$_pos" -lt "${#_buf}" ]; then
              _pos=$(( _pos + 1 ))
              command builtin printf '\033[1C'
            fi
            ;;
        ...
        case "$_kname" in
          ...
          up)
            _sm_set_index "$(_sm_next_selectable \
              $(( ${_hl_index:-1} <= 1 ? n : _hl_index - 1 )) -1)"
            ;;
          ...
          home)
            ...
            _sm_set_index "$(_sm_next_selectable "$n" -1)"
            ;;
    ...
    case "$key" in
      ...
      $'\x01')
        # Ctrl-A — first item (single) / select-all (multi)
        if $multi; then
          local __i

          _msel_set=()

          for ((__i=1; __i<=n; __i++)); do
            _sm_row_selectable "$__i" &&
              _msel_set[$__i]=1
          done
      ...
      $'\x05')
        $multi || _sm_set_index "$(_sm_next_selectable "$n" -1)"
        ;;
...
read() {
    ...
    case "$__smr_f" in
      ...
      -*)
        ...
        while [ -n "$__smr_rest" ]; do
          __smr_c="${__smr_rest:0:1}"
          __smr_rest="${__smr_rest:1}"
          ...
          case "$__smr_c" in
            ...
            p)
              if [ -n "$__smr_rest" ]; then
                _sm_prompt="$__smr_rest"
                __smr_rest=""
              else
                __smr_i=$(( __smr_i + 1 ))
                _sm_prompt="${__smr_a[$__smr_i]}"
              fi
              ;;
  ...
  # caller's variable.
  case "$_sm_var" in
    _sm_*|_vp_*|__smr_*|_buf|_pos|_hl_index|_can_nav|_esc|_kname|_blk_h|REPLY_sm)
      _sm_ok=0
      ;;
  esac
  ...
  [ -n "$_sm_partial" ] &&
    _sm_prompt="$_sm_partial$_sm_prompt"
  ...
  command builtin printf -v "$_sm_var" '%s' "$_sm_result"
